...
def handleOutgoingMail (ctx, mail):
        ...
        xjob = __aps__['x-job']
        if xjob:
                if not uid is None and uid.mailingID > 0:
                        mail.head.append('X-job: ' + xjob % str(uid.mailingID))

        listid = __aps__['list-id']
        if listid:
                if not uid is None and uid.mailingID > 0:
                        mail.head.append('List-ID: ' + listid % (str(uid.companyID), domain))

        abusereports = __aps__['abuse-reports-to']
        if abusereports:
                mail.head.append('X-Abuse-Reports-To: ' + abusereports % domain)
...
if __name__ == '__main__':
        def _main ():
                ...
                mail = struct ()
                mail.head = []
                mail.head.append('Message-ID: <20130823020049-1.1.h.b.0.pp89lw4y80@news.gnlv.fr>')
